...
void DynamixelController::timerCallback(const ros::TimerEvent& e) {
#ifdef DEBUG
  static double priv_read_secs = ros::Time::now().toSec();
#endif
  bool result = false;
...
int main(int argc, char** argv) {
  ...
  ros::Timer time_timer = node_handle.createTimer(ros::Duration(dynamixel_controller.getTimePeriod()), &DynamixelController::timerCallback, &dynamixel_controller);
  ...
  fcntl(fileno(stdin), F_SETFL, O_NONBLOCK); /* non blockingにする */
  ...
  while (1) {
    ...
    if (key != EOF) {
      ...
      //ROS_INFO("non");
      if (errno != EAGAIN)
        break; /* EAGAIN: no resouce is available */
    }
    if (key == 'q')
      break; // qで終了
  ...
  tcsetattr(fileno(stdin), TCSANOW, &save_settings); /*non blockingモードから元に戻す*/
  return 0;
}